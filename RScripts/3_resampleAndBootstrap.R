...
# (in comparison to the whole of wild allelic diversity) is calculated. This is done for each
# level of loci bootstrapping (loci levels specified in the MSAT_lociLevels and DNA_lociLevels variables).
# The result is a list (6 simulation scenarios) of lists (5 genind objects per scenario) of lists 
# (5 levels of loci subset to for each genind object). For each level of loci, 5 sets of allelic
# diversity metrics are calculated (the num_reps variable)
...
# Specify number of resampling replicates, to use for all scenarios below. Note that this will determine
# the number of slices of each resampling array (of which one will exist for each level of loci bootstrapping)
num_reps <- 5
# Specify the loci levels to subset to, for both marker types. These will be used to bootstrap simulated
# genind objects to a random set of loci, prior to resampling
MSAT_lociLevels <- c(5, 10, 15, 20, 25)
DNA_lociLevels <- c(100, 250, 500, 750, 1000)
# Pick plot colors (for all plots!)
plotColors <- c('red','red4','darkorange3','coral','purple')
# Flags for different datasets (marker=msat, dna; nInd=1200, 4800; different n_to_drop flags; DNA low mutation)
MSAT_Flag <- TRUE
DNA_Flag <- TRUE
nInd_1200_Flag <- TRUE
nInd_4800_Flag <- TRUE
...
if(nInd_1200_Flag==TRUE){
  ...
  if(N0_Flag==TRUE){
    ...
    if(MSAT_Flag==TRUE){
      ...
      N1200_MSAT_N0_resamplingArrays <- 
        mclapply(MSAT_geninds, ResampleAndBootstrap_genList, mc.cores=num_cores, lociLevels=MSAT_lociLevels)
      # Name items in the list according to simulation names (in @other slot)
      names(N1200_MSAT_N0_resamplingArrays) <- unlist(lapply(MSAT_geninds, function(x) x[[1]]@other))
      # Save resampling array list to disk
    ...
    if(DNA_Flag==TRUE){
      ...
      # Run resampling in parallel, and save the resampling array result to specified location
      N1200_DNA_N0_resamplingArrays <- 
        mclapply(DNA_geninds, ResampleAndBootstrap_genList, mc.cores=num_cores, lociLevels=DNA_lociLevels)
      # Name items in the list according to simulation names (in @other slot)
      names(N1200_DNA_N0_resamplingArrays) <- unlist(lapply(DNA_geninds, function(x) x[[1]]@other))
      # Save resampling array list to disk
  ...
  if(N1_Flag==TRUE){
    ...
    if(MSAT_Flag==TRUE){
      ...
      # Run resampling in parallel, and save the resampling array result to specified location
      N1200_MSAT_N1_resamplingArrays <- 
        mclapply(MSAT_geninds, ResampleAndBootstrap_genList, mc.cores=num_cores, 
                 lociLevels=MSAT_lociLevels, n_to_drop=1)
      # Name items in the list according to simulation names (in @other slot)
      names(N1200_MSAT_N1_resamplingArrays) <- unlist(lapply(MSAT_geninds, function(x) x[[1]]@other))
      # Save resampling array list to disk
    ...
    if(DNA_Flag==TRUE){
      ...
      # Run resampling in parallel, and save the resampling array result to specified location
      N1200_DNA_N1_resamplingArrays <- 
        mclapply(DNA_geninds, ResampleAndBootstrap_genList, mc.cores=num_cores, 
                 lociLevels=DNA_lociLevels, n_to_drop=1)
      # Name items in the list according to simulation names (in @other slot)
      names(N1200_DNA_N1_resamplingArrays) <- unlist(lapply(DNA_geninds, function(x) x[[1]]@other))
      # Save resampling array list to disk
  ...
  if(N2_Flag==TRUE){
    ...
    if(MSAT_Flag==TRUE){
      ...
      # Run resampling in parallel, and save the resampling array result to specified location
      N1200_MSAT_N2_resamplingArrays <- 
        mclapply(MSAT_geninds, ResampleAndBootstrap_genList, mc.cores=num_cores, 
                 lociLevels=MSAT_lociLevels, n_to_drop=2)
      # Name items in the list according to simulation names (in @other slot)
      names(N1200_MSAT_N2_resamplingArrays) <- unlist(lapply(MSAT_geninds, function(x) x[[1]]@other))
      # Save resampling array list to disk
    ...
    if(DNA_Flag==TRUE){
      ...
      # Run resampling in parallel, and save the resampling array result to specified location
      N1200_DNA_N2_resamplingArrays <- 
        mclapply(DNA_geninds, ResampleAndBootstrap_genList, mc.cores=num_cores, 
                 lociLevels=DNA_lociLevels, n_to_drop=2)
      # Name items in the list according to simulation names (in @other slot)
      names(N1200_DNA_N2_resamplingArrays) <- unlist(lapply(DNA_geninds, function(x) x[[1]]@other))
      # Save resampling array list to disk
...
if(nInd_4800_Flag==TRUE){
  ...
  if(N0_Flag==TRUE){
    ...
    if(MSAT_Flag==TRUE){
      ...
      # Run resampling in parallel, and save the resampling array result to specified location
      N4800_MSAT_N0_resamplingArrays <- 
        mclapply(MSAT_geninds, ResampleAndBootstrap_genList, mc.cores=num_cores, lociLevels=MSAT_lociLevels)
      # Name items in the list according to simulation names (in @other slot)
      names(N4800_MSAT_N0_resamplingArrays) <- unlist(lapply(MSAT_geninds, function(x) x[[1]]@other))
      # Save resampling array list to disk
    ...
    if(DNA_Flag==TRUE){
      ...
      # Run resampling in parallel, and save the resampling array result to specified location
      N4800_DNA_N0_resamplingArrays <- 
        mclapply(DNA_geninds, ResampleAndBootstrap_genList, mc.cores=num_cores, lociLevels=DNA_lociLevels)
      # Name items in the list according to simulation names (in @other slot)
      names(N4800_DNA_N0_resamplingArrays) <- unlist(lapply(DNA_geninds, function(x) x[[1]]@other))
      # Save resampling array list to disk
  ...
  if(N1_Flag==TRUE){
    ...
    if(MSAT_Flag==TRUE){
      ...
      # Run resampling in parallel, and save the resampling array result to specified location
      N4800_MSAT_N1_resamplingArrays <- 
        mclapply(MSAT_geninds, ResampleAndBootstrap_genList, mc.cores=num_cores, 
                 lociLevels=MSAT_lociLevels, n_to_drop=1)
      # Name items in the list according to simulation names (in @other slot)
      names(N4800_MSAT_N1_resamplingArrays) <- unlist(lapply(MSAT_geninds, function(x) x[[1]]@other))
      # Save resampling array list to disk
    ...
    if(DNA_Flag==TRUE){
      ...
      # Run resampling in parallel, and save the resampling array result to specified location
      N4800_DNA_N1_resamplingArrays <- 
        mclapply(DNA_geninds, ResampleAndBootstrap_genList, mc.cores=num_cores, 
                 lociLevels=DNA_lociLevels, n_to_drop=1)
      # Name items in the list according to simulation names (in @other slot)
      names(N4800_DNA_N1_resamplingArrays) <- unlist(lapply(DNA_geninds, function(x) x[[1]]@other))
      # Save resampling array list to disk
  ...
  if(N2_Flag==TRUE){
    ...
    if(MSAT_Flag==TRUE){
      ...
      # Run resampling in parallel, and save the resampling array result to specified location
      N4800_MSAT_N2_resamplingArrays <- 
        mclapply(MSAT_geninds, ResampleAndBootstrap_genList, mc.cores=num_cores, 
                 lociLevels=MSAT_lociLevels, n_to_drop=2)
      # Name items in the list according to simulation names (in @other slot)
      names(N4800_MSAT_N2_resamplingArrays) <- unlist(lapply(MSAT_geninds, function(x) x[[1]]@other))
      # Save resampling array list to disk
    ...
    if(DNA_Flag==TRUE){
      ...
      # Run resampling in parallel, and save the resampling array result to specified location
      N4800_DNA_N2_resamplingArrays <- 
        mclapply(DNA_geninds, ResampleAndBootstrap_genList, mc.cores=num_cores, 
                 lociLevels=DNA_lociLevels, n_to_drop=2)
      # Name items in the list according to simulation names (in @other slot)
      names(N4800_DNA_N2_resamplingArrays) <- unlist(lapply(DNA_geninds, function(x) x[[1]]@other))
      # Save resampling array list to disk
...
if(DNA_Flag==TRUE){
  if(dnaLowMut_Flag==TRUE){
    ...
    if(nInd_1200_Flag==TRUE){
      ...
      # Run resampling in parallel, and save the resampling array result to specified location
      N1200_DNA_lowMut_resamplingArrays <- 
        mclapply(DNA_geninds, ResampleAndBootstrap_genList, mc.cores=num_cores, lociLevels=MSAT_lociLevels)
      # Name items in the list according to simulation names (in @other slot)
      names(N1200_DNA_lowMut_resamplingArrays) <- unlist(lapply(DNA_geninds, function(x) x[[1]]@other))
      # Save resampling array list to disk
    ...
    if(nInd_4800_Flag==TRUE){
      ...
      # Run resampling in parallel, and save the resampling array result to specified location
      N4800_DNA_lowMut_resamplingArrays <- 
        mclapply(DNA_geninds, ResampleAndBootstrap_genList, mc.cores=num_cores, lociLevels=DNA_lociLevels)
      # Name items in the list according to simulation names (in @other slot)
      names(N4800_DNA_lowMut_resamplingArrays) <- unlist(lapply(DNA_geninds, function(x) x[[1]]@other))
      # Save resampling array list to disk

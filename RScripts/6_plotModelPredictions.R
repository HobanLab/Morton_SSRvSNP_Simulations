...
par(mfcol=c(2,1), mar=c(3,3,2,1)+0.1, oma=c(3,2,0.01,0.01))
# %%% MSAT
...
plot(nSamples ~ Total, data=N1200_DF[which(N1200_DF$marker == "MSAT"),], 
     ...
     main='MSAT N1200 MSSE Prediction Model')
# Axes labes
mtext(text="Number of Individuals", side=2, line=3, cex=1.2, srt=90)
mtext(text="Allelic representation (%)", side=1, line=2.5, cex=1.2)
...
legend(x=30, y=1300, legend = c("Fitted line", "95% PI"), 
       col = c("blue","purple"), lwd = 2, lty = c(1,2), bty = "n")
# Change outer margin values, and allow for drawing outside plot region
par(mar=c(1.5,3,4,1)+0.1, xpd=NA)
abline(h=-900, col="black", lwd=2) 
par(xpd=FALSE)
# %%% DNA
# Plot samples versus allelic representation
plot(nSamples ~ Total, data=N1200_DF[which(N1200_DF$marker == "DNA"),], 
     ...
     xlim=c(30,100.5), ylim=c(0,1300), cex.axis=1.2, 
     main='DNA N1200 MSSE Prediction Model')
# Axes labes
mtext(text="Number of Individuals", side=2, line=3, cex=1.2, srt=90)
mtext(text="Allelic representation (%)", side=1, line=2.5, cex=1.2)
# Add fitted prediction line and intervals
...
# Specify colors, and stack 2 plots vertically
plotColors <- c(magma(n=12)[[8]], magma(n=12)[[11]])
par(mfcol=c(2,1), mar=c(5,3,3.5,1)+0.1, oma=c(3,2,0.01,0.01))
# %%% MSAT
# Plot samples versus allelic representation
plot(nSamples ~ Total, data=N4800_DF[which(N4800_DF$marker == "MSAT"),], 
     ...
     main='MSAT N4800 MSSE Prediction Model')
# # Axes labels
mtext(text="Number of Individuals", side=2, line=3, cex=1.1, srt=90)
mtext(text="Allelic representation (%)", side=1, line=2.5, cex=1.2)
# Add fitted prediction line and intervals
...
legend(x=30, y=4900, legend = c("Fitted line", "95% PI"), 
       col = c("blue","purple"), lwd = 2, lty = c(1,2), bty = "n")
# Change outer margin values, and allow for drawing outside plot region
par(mar=c(1.5,3,3,1)+0.1, xpd=NA)
abline(h=-8100, col="black", lwd=2) 
par(xpd=FALSE)
# %%% DNA
# Plot samples versus allelic representation
plot(nSamples ~ Total, data=N4800_DF[which(N4800_DF$marker == "DNA"),], 
     ...
     pch=16, col=alpha(plotColors[[2]], 0.01), 
     xlim=c(30,100.5), ylim=c(0,4900), cex.axis=1.2, 
     main='DNA N4800 MSSE Prediction Model')
# Axes labels
mtext(text="Number of Individuals", side=2, line=3, cex=1, srt=90)
mtext(text="Allelic representation (%)", side=1, line=2.5, cex=1.2)
# Add fitted prediction line and intervals
...
# %%% MODEL COMPARISONS: EXPONENTIAL AND CUBIC %%%% ----
# %%% NIND = 1200 %%% ----
# %%% Read in resampling arrays and build results dataframes ----
# %%% MSAT
N1200_MSAT_resampArr_filepath <- 
